...
parser.add_argument('--mm', default=2, type=int)
# parser.add_argument('--nr', default=16, type=int)
parser.add_argument('--nl', default=10.0, type=float)
...
sim_image_folder = os.path.join(args.save, '_'.join(str(i) for i in [args.dataset, args.nt, args.nl, args.ws, args.ps, args.nn, args.loss]))
...
root = os.path.join(args.out_image, '_'.join(
        str(i) for i in [args.dataset, args.nt, args.mm, args.nl, args.loss]))
...
for image_file in image_files:
    ...
    sim_img_path = os.path.join(sim_image_folder,
                                str(image_file).replace("png", "npy").replace("jpg", "npy").replace("tif", "npy"))
