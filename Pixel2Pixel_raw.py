...
parser.add_argument("--dataset", default="HDRNeRF", type=str, help="Dataset name")
parser.add_argument(
    "--GT", default="GT", type=str, help="Folder name for ground truth images"
)
parser.add_argument(
    "--Noisy", default="Noisy", type=str, help="Folder name for noisy images"
)
parser.add_argument(
    "--save", default="./results", type=str, help="Directory to save pixel bank results"
)
parser.add_argument(
    "--out_image",
    default="./results_image",
    type=str,
...
parser.add_argument(
    "--nn", default=16, type=int, help="Number of nearest neighbors to search"
)
parser.add_argument(
    "--mm", default=8, type=int, help="Number of pixel banks to use for training"
)
...
def postprocess_raw(raw):
    ...
    exposure = torch_quantile(raw, 0.99, interpolation="nearest").to(raw.device)
    # "expose" image by mapping the input exposure level to white and clipping.
    srgb = torch.clip(raw / exposure, 0, 1)
    # apply srgb gamma curve to serve as a simple tonemap.
    tonemapped = linear_to_srgb(srgb)
    return srgb, tonemapped
...
def construct_pixel_bank():
    ...
    bank_dir = os.path.join(
        args.save,
        args.dataset,
        "_".join(str(i) for i in [args.ws, args.ps, args.nn]),
    )
    ...
    blk_sz = 50  # Block size for processing
    ...
    for image_file in image_files:
        ...
        img = torch.from_numpy(img).unsqueeze(0)  # Shape: [1, C, H, W]
        ...
        img_unfold = einops.rearrange(
            img_unfold, "b c (h w) -> b c h w", h=H_new, w=W_new
        )
        ...
        for blk_i in range(num_blk_w):
            for blk_j in range(num_blk_h):
                ...
                patch_windows = F.unfold(
                    sub_img_uf_inp, kernel_size=WINDOW_SIZE, padding=0, stride=1
                )
                ...
                img_center = img_center[
                    ...,
                    center_offset : center_offset + blk_sz,
                    center_offset : center_offset + blk_sz,
                ]
                ...
                _, sort_indices = torch.topk(
                    l2_dis, k=NUM_NEIGHBORS, largest=False, sorted=True, dim=-3
                )
                ...
                patch_center = patch_windows_reshape[
                    :, :, patch_windows_reshape.shape[2] // 2, ...
                ]
                topk = torch.gather(
                    patch_center,
                    dim=-3,
        ...
        topk = einops.rearrange(
            topk, "(w1 w2) c k h w -> k c (w2 h) (w1 w)", w1=num_blk_w, w2=num_blk_h
        )
        topk = topk.permute(2, 3, 0, 1)

        elapsed = time.time() - start_time
        print(
            f"Processed {image_file} in {elapsed:.2f} seconds. Pixel bank shape: {topk.shape}"
        )
...
def loss_func(img1, img2, eps=1e-3):
    pred1 = model(img1)
    _, pred_tonemapped = postprocess_raw(pred1)
    tone_curve = pred_tonemapped.detach() + eps
    loss = (((pred_tonemapped - img2) / tone_curve) ** 2).mean()
    return loss
...
def denoise_images():
    ...
    for image_file in gt_files:
        ...
        clean_img_np = np.load(image_path)
        clean_img_tensor = (
            torch.from_numpy(clean_img_np).unsqueeze(0).cuda()
        )  # No extra dimension is added
        ...
        print(
            f"Number of parameters for {image_file}: {sum(p.numel() for p in model.parameters() if p.requires_grad)}"
        )

        optimizer = optim.AdamW(model.parameters(), lr=lr)
        scheduler = MultiStepLR(optimizer, milestones=[5000, 6500, 8000], gamma=0.5)
        ...
        _, out_img_tonemapped = postprocess_raw(out_img)
        out_img_pil = to_pil_image(out_img_tonemapped.squeeze(0))
        out_img_save_path = os.path.join(
            args.out_image, os.path.splitext(image_file)[0] + ".png"
        )
        out_img_pil.save(out_img_save_path)

        _, noisy_img_tonemapped = postprocess_raw(noisy_img)
        noisy_img_pil = to_pil_image(noisy_img_tonemapped.squeeze(0))
        noisy_img_save_path = os.path.join(
            args.out_image, os.path.splitext(image_file)[0] + "_noisy.png"
        )
        noisy_img_pil.save(noisy_img_save_path)

        print(f"Image: {image_file} | PSNR: {PSNR:.2f} dB")
        avg_PSNR += PSNR
